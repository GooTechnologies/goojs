...
require([
	...
	var resourcePath = "../resources/skeleton_/";
	...
	function loadModels (goo) {
		...
		importer.load(resourcePath + 'skeleton.model', resourcePath, {
			onSuccess: function (entities) {
				...
				for ( var i = 0; i < entities.length; i++) {
					...
					if (entity.meshDataComponent && entity.meshDataComponent.meshData.type === MeshData.SKINMESH) {
						setupSkinning(entity);
						loadAnimations(entity.meshDataComponent.meshData.currentPose);
					}
				}
			},
	...
	function setupSkinning(entity) {
		var meshData = entity.meshDataComponent.meshData;
		var skinShader = Material.createShader(Util.clone(ShaderLib.skinning));

		skinShader.defines.JOINT_COUNT = meshData.paletteMap.length;
		skinShader.defines.WEIGHTS = meshData.weightsPerVertex;

		entity.meshRendererComponent.materials[0].shader = skinShader;
	}

	function loadAnimations(pose) {
		var treeLoader = new AnimationTreeLoader({
			loader: new Loader({
				rootPath: resourcePath
			})
		});

		treeLoader.load('skeleton.anim', pose, "walk_anim")
