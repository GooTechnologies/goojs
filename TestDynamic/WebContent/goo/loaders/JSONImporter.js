define(
	['goo/entities/components/TransformComponent', 'goo/renderer/MeshData', 'goo/loaders/JsonUtils', 'goo/entities/components/MeshDataComponent',
			...
			'goo/util/URLTools'], function(TransformComponent, MeshData, JsonUtils, MeshDataComponent, MeshRendererComponent, Material,
		TextureCreator, Shader, Joint, Skeleton, SkeletonPose, AnimationClip, JointChannel, TransformChannel, InterpolatedFloatChannel, URLTools) {
		"use strict";
		...
		JSONImporter.prototype._parseMeshData = function(object, weightsPerVert, entity, type) {
			...
			if (object.Normals) {
				if (this.useCompression) {
					...
					JsonUtils.fillAttributeBufferFromCompressedString(object.Normals, meshData, MeshData.NORMAL, [scale, scale, scale], [offset,
							offset, offset]);
				} else {
			...
			if (object.Tangents) {
				if (this.useCompression) {
					...
					JsonUtils.fillAttributeBufferFromCompressedString(object.Tangents, meshData, MeshData.TANGENT, [scale, scale, scale, scale], [
					...
					for ( var key in this.slotUnitMap) {
						if (info.textureFileNames[key] !== undefined) {
							...
							if (this.nameResolver !== undefined) {
								tex = new TextureCreator().withMinificationFilter(minificationFilter).withVerticalFlip(flipTexture)
									.withGooResourceCache(_useCache).makeTexture2D(nameResolver.resolveName(baseTexFileName));
							} else {
