require([
	...
	function createEntity(goo, meshData, ammoSettings, pos, collider, mat, name) {
	...
	function rayCast() {
		...
		var result = ammoWorkerSystem.rayCast(start, end);
	}
	...
	function addPlane() {
		var meshData = new Quad(10, 10);
		//var material = V.getColoredMaterial();
		var entity = goo.world.createEntity(meshData, material, [0, -5, 0]).addToWorld();
		entity.setRotation([-Math.PI/2, 0, 0]);
		entity.setComponent(new AmmoWorkerRigidbodyComponent({
			mass: 0 // static
		}));
		entity.name = 'terrain';
		entity.setComponent(new ColliderComponent(new BoxCollider({
			halfExtents: new Vector3(100,100,1),
		})));
		...
		colliderEntity.addToWorld();
		entity.attachChild(colliderEntity);
		colliderEntity.transformComponent.transform.translation.setd(0, 0, 0);
		colliderEntity.transformComponent.setUpdated();
		colliderEntity.setComponent(new ColliderComponent(new PlaneCollider({
