 ...
 */
class Cylinder extends MeshData {
	radialSamples: number;
	radiusTop: number;
	radiusBottom: number;
	height: number;

	constructor(radialSamples: number = 8, radiusTop: number = 0.5, radiusBottom: number = 0.5, height: number = 1) {
		super(MeshData.defaultMap([MeshData.POSITION, MeshData.NORMAL, MeshData.TEXCOORD0]), radialSamples * 4 + 2 + 2, (radialSamples * 3) * 4);
	 ...
	 */
	clone() {
		return new Cylinder(this.radialSamples, this.radiusTop, this.radiusBottom, this.height);
	};
}
