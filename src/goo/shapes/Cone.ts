 ...
 */
class Cone extends MeshData {
	radialSamples: number;
	radius: number;
	height: number;
	constructor(radialSamples, radius, height) {
		super(MeshData.defaultMap([MeshData.POSITION, MeshData.NORMAL, MeshData.TEXCOORD0]), radialSamples * 3 + radialSamples + 1, radialSamples * 3 * 2);
	 ...
	 */
	clone() {
		return new Cone(this.radialSamples, this.radius, this.height);
	};
}
