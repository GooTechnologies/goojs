 ...
 */
class Disk extends MeshData {
	nSegments: number;
	radius: number;
	pointiness: number;

	constructor(nSegments, radius, pointiness) {
		super(MeshData.defaultMap([MeshData.POSITION, MeshData.NORMAL, MeshData.TEXCOORD0]), nSegments + 1, nSegments * 3);
	 ...
	 */
	clone() {
		return new Disk(this.nSegments, this.radius, this.pointiness);
	};
}
