...
class StringUtils {
	 ...
	 */
	static getUntil(string, stopString) {
	 ...
	 * @returns {string}
	 */
	static getAfterLast(string, stopString) {
	 ...
	 */
	static getFrom(string, startString) {
	...
	static getIndexedName(base, takenNames, separator) {
		if (!separator) {
			separator = '_';
		}
	...
	static getUniqueName(desiredName, takenNames, separator) {
	...
	*/
	static hashCode(str) {
		...
		return btoa(hash.toString()).replace('/', '_').replace('+', '-');
	};
