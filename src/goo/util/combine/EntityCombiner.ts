...
class EntityCombiner {
	...
	constructor(gooWorld, gridCount, removeOldData, keepEntities) {
		this.world = gooWorld;
	...
	_buildSubs(entity, baseSubs, subs) {
	...
	_combine(root, combineList) {
		...
		for (var i = 0; i < combineList.length; i++) {
			...
			var key2: any = Object.keys(attributeMap);
			key2.sort();
			key2 = key2.join('_');
