...
class ObjectUtils {
	 ...
	 */
	static defaults(destination, source) {
	 ...
	 * @returns {Object} Returns the destination object
	 */
	static copyOptions(destination, options, defaults) {
	...
	static forEach(obj, iterator, context, sortProp) {
	 ...
	 * @returns {Array}
	 */
	static map(collection, iteratee, context, sortProp) {
		var result = [];
	...
	static shallowSelectiveClone(source, keys) {
		var clone = {};
