		...
		PhysicsSystem.prototype.updateLonelyColliders = function (forceUpdate) {
			for (var i = this._activeColliderEntities.length - 1; i >= 0; i--) {
				...
				if (!colliderComponent.lastScale) {
					...
					colliderComponent.lastScale.setVector(entity.transformComponent.worldTransform.scale);
				}

				if (colliderComponent && (colliderComponent._dirty)) {
					...
					if (body) {
						...
						if (cannonShape) {
							...
							if (colliderComponent.lastScale.distanceSquared(entity.transformComponent.worldTransform.scale) !== 0) {
