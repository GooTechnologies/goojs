...
class ValueChannel extends AbstractTimelineChannel {
	value: any;
	callbackUpdate: any;
	callbackEnd: any;
	 ...
	 */
	addKeyframe(id, time, value, easingFunction) {
		var newKeyframe = {
			id: id,
			time: time,
			value: value,
	...
	// tween factories
	static getSimpleTransformTweener = function (type, vectorComponent, entityId, resolver) {
		var entity;
	...
	static getRotationTweener = function (angleIndex, entityId, resolver, rotation) {
		var entity;
		var func: any = function (time, value) {
			if (!entity) { entity = resolver(entityId); }
