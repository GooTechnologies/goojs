...
class ScriptUtils {
	...
	static isRefType = function (type) {
		return ObjectUtils.contains(ScriptUtils.REF_TYPES, type);
	};

	static TYPE_VALIDATORS = (function () {
	...
	static PARAMETER_CONTROLS = (function () {
		var typeControls = {
			'string': ['key'],
	 ...
	 */
	static fillDefaultValues = function (parameters, specs) {
	 ...
	 * @param specs {Array<{key, name, default, description}>}
	 */
	static fillDefaultNames = function (specs) {
		if (!(specs instanceof Array)) { return; }
	...
	static _keyInverse = (function (assoc) {
		var inverseAssoc = {};
