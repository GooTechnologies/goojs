...
class MeshData {
	...
	type: any;
	paletteMap: any;
	weightsPerVertex: any;
	boundingBox: any;
	store: any;
	wireframeData: any;
	flatMeshData: any;
	__boundingTree: any;
	_attributeDataNeedsRefresh: any;
	_dirtyAttributeNames: any;
	_vertexCountStore: any;

	constructor(attributeMap, vertexCount?, indexCount?) {
	 ...
	 */
	rebuildData(vertexCount, indexCount, saveOldData=false) {
		var savedAttributes = {};
	 ...
	 * @returns {array}
	 */
	getPrimitiveVertices(primitiveIndex, section, store) {
		...
		for (var i = 0; i < rSize; i++) {
			...
			if (this.getIndexBuffer()) {
				...
				var v = this.getVertexIndex(primitiveIndex, i, section);
				result[i].x = verts[v * 3 + 0];
				result[i].y = verts[v * 3 + 1];
				result[i].z = verts[v * 3 + 2];
			}
		}
	...
	getVertexIndex(primitiveIndex, point, section) {
		var index = 0;
		...
		switch (this.indexModes[section]) {
			...
			case 'LineLoop':
				...
				break;
			default:
				console.warn('unimplemented index mode: ' + this.indexModes[section]);
				return -1;
		}
	...
	static getPrimitiveCount(indexMode, size) {
		switch (indexMode) {
			case 'Triangles':
				return size / 3;
			case 'TriangleFan':
	...
	static getVertexCount(indexMode) {
		switch (indexMode) {
			case 'Triangles':
			case 'TriangleFan':
			case 'TriangleStrip':
	...
	makeInterleavedData() {
		...
		for (var key in this.attributeMap) {
			...
			var offset = attribute.offset as number;
	...
	getAttributeBuffer(attributeName) {
	 ...
	 */
	applyTransform(attributeName, transform) {
	 ...
	 * @returns {MeshData} Self to allow chaining
	 */
	applyFunction(attributeName, fun) {
	 ...
	 * @readonly
	 */
	static TEXCOORD0 = 'TEXCOORD0';
	/**
	 * @type {string}
	 * @readonly
	 */
	static TEXCOORD1 = 'TEXCOORD1';
	/**
	 * @type {string}
	 * @readonly
	 */
	static TEXCOORD2 = 'TEXCOORD2';
	/**
	 * @type {string}
	 * @readonly
	 */
	static TEXCOORD3 = 'TEXCOORD3';
	/**
	 * @type {string}
	 * @readonly
	 */
	 ...
	 */
	static createAttribute(count, type, normalized?) {
		return {
			count: count,
			type: type,
			stride: 0,
	 ...
	 */
	static defaultMap(types) {
		if (types === undefined) {
			return MeshData.buildMap(Object.keys(getDefaults()));
		} else {
			return MeshData.buildMap(types);
		}
	};
}
