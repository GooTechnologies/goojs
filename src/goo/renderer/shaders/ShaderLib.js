	...
	ShaderLib.uber = {
		...
		vshader: function () {
			...
			'void main(void) {',
				...
				'#endif',
				'#ifdef TEXCOORD0',
				'	texCoord0Orig = vertexUV0 * offsetRepeat.zw + offsetRepeat.xy;',
				'#endif',
				'#ifdef TEXCOORD1',
				'	texCoord1 = vertexUV1;',
				'#endif',
		...
		fshader: function () {
			...
			'#ifdef HEIGHT_MAP',
				'vec3 reliefMapping(sampler2D relief_map, in vec3 ray_pos, in vec3 ray_dir) {',
					...
					'for(int i = 0; i < cone_steps; i++) {',
						...
						'float height = clamp(tex.w - pos.z, 0.0, 1.0);',
				...
				'vec2 parallaxMapping(sampler2D u_heightTexture, in vec3 camInTangent, in vec2 o_texcoords, in float parallaxScale) {',
					'float initialHeight = texture2D(u_heightTexture, o_texcoords).r;',
					'vec2 texCoordOffset = parallaxScale * camInTangent.xy * initialHeight / camInTangent.z;',
			...
			'void main(void) {',
				...
				'#if defined(TANGENT) && defined(HEIGHT_MAP) && defined(TEXCOORD0)',
					'mat3 worldToTangent = mat3(tangent, binormal, normal);',
					'vec3 camInTan = (normalize(viewPosition) * worldToTangent);',

					// 'vec2 texCoord0 = parallaxMapping(heightMap, camInTan, texCoord0Orig, 0.03);',

					'camInTan.xy = -camInTan.xy;',
					'vec2 texCoord0 = reliefMapping(heightMap, vec3(texCoord0Orig, 0.0), camInTan).xy;',
	
					// 'vec2 coor = reliefMapping(heightMap, vec3(texCoord0Orig, 0.0), camInTan).xy;',
					// 'final_color.rg = coor;',
	
					// 'final_color = vec4(texture2D(heightMap, texCoord0Orig).rrr, 1.0);',
					// 'vec2 texCoord0 = texCoord0Orig;',
