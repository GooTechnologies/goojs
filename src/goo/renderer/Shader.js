define([
	...
	Shader.prototype.compile = function (renderer) {
		...
		this.vertexShader = this._getShader(context, context.VERTEX_SHADER, this.vertexSource);
		this.fragmentShader = this._getShader(context, context.FRAGMENT_SHADER, this.fragmentSource);
		...
		if (this.shaderProgram === null || error !== context.NO_ERROR) {
	...
	Shader.prototype._getShader = function (context, type, source) {
		...
		if (!context.getShaderParameter(shader, context.COMPILE_STATUS)) {
			...
			var shaderType = type === context.VERTEX_SHADER ? 'VertexShader' : 'FragmentShader';
